...
export class HoursComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.addSpacer(this.hoursOfOp, "HofOp");
    if (this.hoursServing) {
      this.addSpacer(this.hoursServing, "serving");
    }
  }

  addSpacer(data, arrayName) {
    if (arrayName == "HofOp") {
      Object.keys(data).forEach(day => {
        data[day].day = day;
        if (data[day].isClosed === true) {
          data[day].spacer = "Closed";
        } else {
          data[day].spacer = "-";
        }
        this.HofOp.push(data[day]);
      });
    } else if (arrayName == "serving") {
      Object.keys(data).forEach(day => {
        data[day].day = day;
        if (data[day].isClosed === true) {
          data[day].spacer = "Closed";
